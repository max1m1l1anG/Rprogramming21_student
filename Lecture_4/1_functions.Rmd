...
```{r}
# your code here
df_2018 <- readRDS("~/BSc Economics - TiU/Semester 2/Programming in R and Python/Assignments/R/Programming21_student_V2/Rprogramming21_student/Lecture_4/sourcedata/clothing_exports_aggregated.rds")
...
plot_top_five_receiving_countries <- function(dataframe, country_code){
  # code from the previous chunk adjusted to deal with any given country code
  df_tmp <- dataframe %>% filter(reporter_iso == as.character(country_code)) %>% 
  group_by(year) %>% slice_max(order_by = export_value_usd, n = 5)
...
# your code here
for (i in countries) {
  print(plot_top_five_receiving_countries(df_2018, i))
}
```
