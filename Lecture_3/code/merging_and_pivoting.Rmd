...
```{r}
# Here your code
data12_merge <- merge(data1, data2, by = "Hospital")
...
```{r}
# Here your code
data12_fulljoin <- full_join(data1, data2, by = "Hospital")
data12_fulljoin
```
...
```{r}
# Here your code
data12_innerjoin <- inner_join(data1, data2, by = "Hospital")
data12_innerjoin
```
...
```{r}
# Here your code
data12_leftjoin <- left_join(data1, data2, by = "Hospital")
data12_leftjoin
```
...
```{r}
# Here your code
data12_rightjoin <- right_join(data1, data2, by = "Hospital")
data12_rightjoin
...
```{r}
# Here your code
data12_antijoin <- anti_join(data1, data2, by = "Hospital")
data12_antijoin
...
# Here your code
data12_semijoin <- semi_join(data1, data2, by = "Hospital")
data12_semijoin
```
...
```{r}
# Here your code
data12_cbind <- cbind(data1, data2)
data12_cbind
```
...
```{r}
# Here your code
data12_rbind <- rbind(data1a, data1)
data12_rbind
```
...
# Here your code
df1 <- read_excel("~/BSc Economics - TiU/Semester 2/Programming in R and Python/Assignments/R/Programming21_student_V2/Rprogramming21_student/Lecture_3/sourcedata/OECD-Health-Statistics-2020-Frequently-Requested-Data.xls", 
    sheet = 3, 
    skip = 3)
colnames(df1)[colnames(df1) == "...1"] <- "Country"
colnames(df1)[colnames(df1) == "2019...51"] <- "2019"
colnames(df1)[colnames(df1) == "2019...53"] <- "2019"
colnames(df1)[colnames(df1) == "...52"] <- ""
...
# Here your code
df2 <- pivot_longer(df1, cols = -Country, names_to = "Year", values_to = "Expenditure")
...
# Here your code
ggplot() +
  geom_line(data = df2, aes(x = as.integer(Year), y = Expenditure, color = Country)) +
...
```{r}
# Here your code
df3 <- pivot_wider(df3, names_from = Year, values_from = Expenditure)
...
# Here your code
ggplot() +
  geom_col(data = df3, aes(x = reorder(Country, -Growth), y = Growth, fill = Country), show.legend = FALSE) +
  labs(x = "", y = "Growth") +
  theme_bw()
